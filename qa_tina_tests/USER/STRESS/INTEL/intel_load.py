...
def test_Create_Read_Delete(osc_sdk, queue, args):
    ...
    for _ in range(args.num_read_per_process):
        ...
        try:
            call_number += 1
            ret = osc_sdk.intel.gateway.create(owner=osc_sdk.config.account.account_id)
            net_id = ret.response.result.id
        ...
        if net_id:
            try:
                call_number += 1
                ret = osc_sdk.intel.gateway.find(owner=osc_sdk.config.account.account_id, id=net_id).response.result
                if not ret:
                    net_id = None
            except OscApiException as error:
        ...
        if net_id:
            try:
                call_number += 1
                osc_sdk.intel.gateway.delete(owner=osc_sdk.config.account.account_id, gateway_id=net_id)
                net_id = None
            except OscApiException as error:
...
if __name__ == '__main__':
    ...
    try:
        ...
        for i in range(args.process_number):
            proc = Process(name="load-{}".format(i), target=test_Create_Read_Delete, args=[oscsdk, queue, args])
