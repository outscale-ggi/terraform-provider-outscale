...
def create_account(config, queue, args):
    ...
    for _ in range(args.num_create_per_process):
        ...
        try:
            ...
            pid = account_tools.create_account(OscSdk(config=config), account_info=account_info)
        ...
        if pid:
            try:
                ret = intel.kms.key.find(owner=pid, is_default=True)
                if not ret.response.result and len(ret.response.result) != 1:
                    assert False, 'Could not find default key for new account'
            except OscApiException as error:
                errs.handle_api_exception(error, error_type.Create)
            except Exception as error:
                errs.add_unexpected_error(error)
...
def run(args):
    ...
    for i in range(args.process_number):
        proc = Process(name="load-{}".format(i), target=create_account, args=[config, queue, args])
