...
def create_snapshot(oscsdk, kp_info, inst_info, volId, device, args, queue):
...
if __name__ == '__main__':
    ...
    if not init_error:
        ...
        for i in range(args.desc_snap):
            # def test_snapshot(conn, keyPath, ipAddress, volId, device, args, queue):
            t = Thread(name="pfsbu-{}".format(i + 1), target=describe_snapshot, args=[oscsdk, QUEUE])
            threads.append(t)
            t.start()
        ...
        logger.info("OK = {} - KO = {}".format(NB_OK, NB_KO))
        logger.info("Tries = {} - Errors = {}".format(sucess, errors))
        logger.info("total_time = {} - Mean = {}".format(total_time, total_time / args.desc_snap))
