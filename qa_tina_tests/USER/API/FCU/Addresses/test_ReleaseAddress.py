...
class Test_ReleaseAddress(OscTinaTest):
    ...
    def setup_method(self, method):
        ...
        try:
            ...
            for _ in range(2):
                ...
                ret = self.a1_r1.fcu.DescribeAddresses(PublicIp=ip)
                alloc_id = ret.response.addressesSet[0].allocationId
                self.ip_list.append({'ip': ip, 'alloc_id': alloc_id})
            ret = self.a1_r1.fcu.AssociateAddress(InstanceId=self.inst_info[INSTANCE_ID_LIST][0], PublicIp=self.ip_list[0]['ip'])
            ret = self.a2_r1.fcu.AllocateAddress()
            self.ip_account2 = ret.response.publicIp
            ret = self.a2_r1.fcu.DescribeAddresses(PublicIp=self.ip_account2)
    ...
    def teardown_method(self, method):
        try:
            for conn in [self.a1_r1, self.a2_r1]:
                ret = conn.fcu.DescribeAddresses()
    ...
    def test_T312_with_allocationid_from_another_account(self):
        try:
            self.a1_r1.fcu.ReleaseAddress(AllocationId=self.alloc_id_account2)
    ...
    def test_T316_with_publicip_from_another_account(self):
        try:
            self.a1_r1.fcu.ReleaseAddress(PublicIp=self.ip_account2)
    ...
    def test_T309_with_valid_allocationid(self):
        self.a1_r1.fcu.ReleaseAddress(AllocationId=self.ip_list[1]['alloc_id'])

    def test_T311_with_allocationid_in_use_public_cloud(self):
        self.a1_r1.fcu.ReleaseAddress(AllocationId=self.ip_list[0]['alloc_id'])
    ...
    def test_T310_with_invalid_allocationid(self):
        try:
            self.a1_r1.fcu.ReleaseAddress(AllocationId='toto')
    ...
    def test_T313_with_valid_publicip(self):
        self.a1_r1.fcu.ReleaseAddress(PublicIp=self.ip_list[1]['ip'])

    def test_T314_with_invalid_publicip(self):
        try:
            self.a1_r1.fcu.ReleaseAddress(PublicIp='toto')
    ...
    def test_T315_with_publicip_in_use(self):
        self.a1_r1.fcu.ReleaseAddress(PublicIp=self.ip_list[0]['ip'])

    def test_T317_with_valid_allocationid_and_publicip(self):
        self.a1_r1.fcu.ReleaseAddress(PublicIp=self.ip_list[1]['ip'], AllocationId=self.ip_list[1]['alloc_id'])

    def test_T318_with_invalid_allocationid_and_publicip(self):
        self.a1_r1.fcu.ReleaseAddress(PublicIp=self.ip_list[1]['ip'], AllocationId=self.ip_list[0]['alloc_id'])

    def test_T319_with_invalid_param(self):
        self.a1_r1.fcu.ReleaseAddress(PublicIp=self.ip_list[1]['ip'], toto='toto')
