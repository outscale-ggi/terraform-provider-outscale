...
class Test_DetachVolume(OscTestSuite):
    ...
    def setup_class(cls):
        cls.inst_info = None
        cls.rslt_detach_standard = None
        cls.rslt_detach_gp2 = None
        cls.rslt_detach_io1 = None
        ...
        try:
            ...
            wait_volumes_state(cls.a1_r1, cls.standard_volume_ids, 'available')
            cls.rslt_attach_standard = cls.a1_r1.fcu.AttachVolume(VolumeId=cls.standard_volume_ids[0],
                                                                  InstanceId=cls.inst_info[INSTANCE_ID_LIST][0], Device="/dev/xvdb")
            ...
            cls.rslt_attach_gp2 = cls.a1_r1.fcu.AttachVolume(VolumeId=cls.gp2_volume_ids[0],
                                                             InstanceId=cls.inst_info[INSTANCE_ID_LIST][0], Device="/dev/xvdc")
            cls.rslt_attach_io1 = cls.a1_r1.fcu.AttachVolume(VolumeId=cls.io1_volume_ids[0],
                                                             InstanceId=cls.inst_info[INSTANCE_ID_LIST][0], Device="/dev/xvdd")
        except:
            try:
                cls.teardown_class()
            except:
    ...
    def test_T3960_option_force_gp2(self):
        self.rslt_detach_gp2 = self.a1_r1.fcu.DetachVolume(VolumeId=self.gp2_volume_ids[0],
                                                           InstanceId=self.inst_info[INSTANCE_ID_LIST][0], Force=True)
        wait_volumes_state(self.a1_r1, volume_id_list=[self.gp2_volume_ids[0]], state="available")

    def test_T3961_option_force_io1(self):
        self.rslt_detach_io1 = self.a1_r1.fcu.DetachVolume(VolumeId=self.io1_volume_ids[0],
                                                           InstanceId=self.inst_info[INSTANCE_ID_LIST][0], Force=True)
        wait_volumes_state(self.a1_r1, volume_id_list=[self.io1_volume_ids[0]], state="available")
    ...
    def test_T5028_from_an_other_instance(self):
        try:
            self.rslt_detach_standard = self.a1_r1.fcu.DetachVolume(VolumeId=self.standard_volume_ids[1],
                                                                    InstanceId=self.inst_info[INSTANCE_ID_LIST][1])
            assert False, "Call should not be successful"
        except OscApiException as error:
            assert_error(error, 400, "InvalidParameterValue",
                         "Given Instance {} and Volume {} are not attached".format(self.inst_info[INSTANCE_ID_LIST][1], self.standard_volume_ids[1]))
