...
class Test_ReEncryptCiphertext(OKMS):
    ...
    def setup_class(cls):
        ...
        try:
            cls.master_key = cls.a1_r1.oapi.CreateMasterKey().response.MasterKey.MasterKeyId
        except Exception:
            try:
                cls.teardown_class()
            except Exception:
    ...
    def setup_method(self, method):
        ...
        self.orig_master_key = None
        try:
            self.orig_master_key = self.a1_r1.oapi.CreateMasterKey().response.KeyMetadata.MasterKey.MasterKeyId
            self.orig_encryption_context = {'name': 'value'}
            encoded_text = base64.b64encode(id_generator(size=128).encode('utf-8')).decode('utf-8')
            self.cipher_text = self.a1_r1.oapi.EncryptPlaintext(KeyId=self.orig_master_key, Plaintext=encoded_text, EncryptionContext=self.orig_encryption_context).response.CipherText
        except:
            try:
    ...
    def teardown_method(self, method):
        try:
            if self.orig_master_key:
                self.a1_r1.oapi.DeleteMasterKey(KeyId=self.orig_master_key, PendingWindowInDays=7)
        finally:
