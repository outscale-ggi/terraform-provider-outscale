...
class Test_EncryptPlaintext(OKMS):
    ...
    def teardown_class(cls):
        try:
            if cls.master_key:
                try:
                    cls.a1_r1.oapi.DeleteMasterKey(KeyId=cls.master_key.MasterKeyId, PendingWindowInDays=7)
                except:
                    pass
        finally:
    ...
    def test_T5169_valid_params(self):
        ...
        resp = self.a1_r1.oapi.EncryptPlaintext(KeyId=self.master_key.MasterKeyId, Plaintext=encoded_text).response
    ...
    def test_T5171_missing_plain_text(self):
        try:
            self.a1_r1.oapi.EncryptPlaintext(KeyId=self.master_key.MasterKeyId)
    ...
    def test_T5172_incorrect_key_id(self):
        try:
            encoded_text = base64.b64encode(id_generator(size=128).encode('utf-8')).decode('utf-8')
            self.a1_r1.oapi.EncryptPlaintext(KeyId='cmk-11111111', Plaintext=encoded_text)
    ...
    def test_T5173_too_short_plain_text(self):
        try:
            ...
            self.a1_r1.oapi.EncryptPlaintext(KeyId=self.master_key.MasterKeyId, Plaintext=encoded_text)
    ...
    def test_T5175_with_encryption_context(self):
        encoded_text = base64.b64encode(id_generator(size=128).encode('utf-8')).decode('utf-8')
        ret = self.a1_r1.oapi.EncryptPlaintext(KeyId=self.master_key.MasterKeyId, Plaintext=encoded_text, EncryptionContext={'name': 'value'})
        assert ret.response.Ciphertext

    def test_T5176_other_account(self):
        try:
            encoded_text = base64.b64encode(id_generator(size=128).encode('utf-8')).decode('utf-8')
            self.a2_r1.oapi.EncryptPlaintext(KeyId=self.master_key.MasterKeyId, Plaintext=encoded_text)
            assert False, 'Call should not have been successful, incorrect key owner'
        except OscApiException as error:
            assert_error(error, 400, 'NotFoundException', 'The customer master key does not exist: {}'.format(self.master_key.MasterKeyId))

    def test_T5177_with_standard_special_characters(self):
        text = "Th!$ !$ @ text w!th $pec!@£ ch@r@cter$!"
        encoded_text = base64.b64encode(text.encode('utf-8')).decode('utf-8')
        ret = self.a1_r1.oapi.EncryptPlaintext(KeyId=self.master_key.MasterKeyId, Plaintext=encoded_text)
        assert ret.response.Ciphertext
    ...
    def test_T5179_dry_run(self):
        encoded_text = base64.b64encode(id_generator(size=128).encode('utf-8')).decode('utf-8')
        ret = self.a1_r1.oapi.EncryptPlaintext(KeyId=self.master_key.MasterKeyId, Plaintext=encoded_text, DryRun=True)
        assert_dry_run(ret)
