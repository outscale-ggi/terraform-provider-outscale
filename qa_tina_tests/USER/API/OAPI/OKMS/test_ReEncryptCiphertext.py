...
#             self.cipher_text = self.a1_r1.oapi.EncryptPlaintext(MasterKeyId=self.orig_master_key_id, Plaintext=encoded_text,
#                    EncryptionContext=self.orig_encryption_context).response.Ciphertext
#         except:
#             try:
...
#         resp = self.a1_r1.oapi.ReEncryptCiphertext(NewMasterKeyId=self.master_key_id, OriginalCiphertext=self.cipher_text,
#                OriginalEncryptionContext=self.orig_encryption_context)
#         ret.check_response()
...
#         try:
#             self.a1_r1.oapi.ReEncryptCiphertext(OriginalCiphertext=self.cipher_text,
#                OriginalEncryptionContext=self.orig_encryption_context).response
...
#         try:
#             self.a1_r1.oapi.ReEncryptCiphertext(NewMasterKeyId=self.master_key_id, OriginalCiphertext='toto',
#            OriginalEncryptionContext=self.orig_encryption_context).response
#             assert False, 'Call should not have been successful, missing key id'
#         except OscApiException as error:
#             assert_error(error, 400, '', '')
...
#         try:
#             self.a1_r1.oapi.ReEncryptCiphertext(NewMasterKeyId=self.master_key_id, OriginalCiphertext='toto',
#            OriginalEncryptionContext={'foo': 'bar'}).response
#             assert False, 'Call should not have been successful, missing key id'
#         except OscApiException as error:
#             assert_error(error, 400, '', '')
#
#     def test_T5211_dry_run(self):
#         ret = self.a1_r1.oapi.ReEncryptCiphertext(NewMasterKeyId=self.master_key_id, OriginalCiphertext=self.cipher_text,
#            OriginalEncryptionContext=self.orig_encryption_context, DryRun=True)
#         assert_dry_run(ret)
