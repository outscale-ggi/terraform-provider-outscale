...
def perf_storage(oscsdk, service, logger, queue, args):
    ...
    try:
        ...
        result["create_bucket"] = numpy.array(durations).mean()
        ...
        result["put_object"] = numpy.array(durations).mean()
        ...
        if obj_names:
            ...
            for obj_name in obj_names:
                ...
                durations.append((datetime.now() - start_desc).total_seconds())
            result["delete_object"] = numpy.array(durations).mean()
        if bucket_names:
            num = 0
            durations = []
            for bucket_name in bucket_names:
                ...
                durations.append((datetime.now() - start_desc).total_seconds())
                num += 1
            result["delete_bucket"] = numpy.array(durations).mean()
