...
class Test_firewall_vpc(OscTestSuite):
    ...
    def test_T1533_spwan_firewall_VPC(self):
        sshclient = SshTools.check_connection_paramiko(
            self.public_ip, self.vpc_info[KEY_PAIR][PATH], username=self.a1_r1.config.region.get_info(constants.CENTOS_USER)
        )
        ...
        sshclient = SshTools.check_connection_paramiko(
            self.public_ip, self.vpc_info[KEY_PAIR][PATH], username=self.a1_r1.config.region.get_info(constants.CENTOS_USER), retry=20, timeout=10
        )
