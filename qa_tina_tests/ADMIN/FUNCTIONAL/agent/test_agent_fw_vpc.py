...
class Test_agent_fw_vpc(OscTestSuite):
    ...
    def check_ngw_ping(self, pub_ip, kp_path, local_addr, dest_addr):
        sshclient = SshTools.check_connection_paramiko(
            pub_ip, kp_path, username=self.a1_r1.config.region.get_info(constants.CENTOS_USER), retry=4, timeout=10
        )
    ...
    def test_T1924_agent_fw_vpc_restart(self):
        ...
        sshclient = SshTools.check_connection_paramiko(
            inst_ip, os.path.expanduser(self.a1_r1.config.region.get_info(constants.FW_KP)), username='root'
        )
