    ...
    try:
        for snap_cfg in DEFAULT_SNAP_CFG:
            ...
            for i in range(snap_cfg[0]):
                ...
                write_and_snap(osc_sdk=osc_sdk, sshclient=sshclient, inst_id=inst_info[INSTANCE_ID_LIST][0], vol_id=vol_id, f_num=i,
                               w_size=snap_cfg[2], fio=snap_cfg[3], snap_name=snap_name, snap_attached=True if (i % 2) == 0 else False)
            # delete_vol
...
if __name__ == '__main__':
    ...
    args_p = argparse.ArgumentParser(description="Setup ressources for streaming tests",
                                     formatter_class=argparse.RawTextHelpFormatter)

    args_p.add_argument('-r', '--region-az', dest='az', action='store',
                        required=True, type=str, help='Selected Outscale region')
    args_p.add_argument('-a', '--account', dest='account', action='store',
                        required=True, type=str, help='Set account used')
    args_p.add_argument('-c', '--clean', dest='clean', action='store_true',
                        help='Clean Account ressources')
    ...
    setup_streaming_ressources(osc_sdk)
